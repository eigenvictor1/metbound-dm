...
paramsHiggsinoBino = { L[x], YT[x], YB[x], YTA[x], g[x], gp[x], gs[x], yL[x], yR[x] };
paramsHiggsinoBinoSubs =  { L -> L[x], YT -> YT[x], YB -> YB[x], YTA -> YTA[x], g -> g[x], gp -> gp[x], gs -> gs[x], yL -> yL[x], yR -> yR[x] };
...
ThresholdHiggsinoBino[{L_, YT_, YB_, YTA_, g_, gp_, gs_, yL_, yR_, mD_, mS_}]:=
	Module[{x},
...
LOneLoopHiggsinoBino[{L_, YT_, YB_, YTA_, g_, gp_, gs_, yL_, yR_, mD_, mS_, x_}]:=(4/(2048 Pi^2) (-5 gp^4+6 (gp^2+g^2)^2 Log[(gp^2+g^2)/4]-10 gp^2 g^2-15 gp^4+12 g^4 Log[g^2/4]+144 YT^4-96 YT^4 Log[YT^2/2])-1/(8\[Pi]^2) Sum[(massvals[mD,mS,yL,yR,x][[i]])^2/Exp[4x](Log[(massvals[mD,mS,yL,yR,x][[i]]+0.0000000001)/Exp[2x]]-3/2),{i,1,3}])
...
LOneLoopHiggsinoBino[{\[Lambda], yt, yb , y\[Tau], g2, g1, g3, y1, y2, mD, mS, x}]
